...
object QuickDiffArgs {
  ...
  def parseArgs(
      argList: List[String],
      quickDiffArgs: QuickDiffArgs = QuickDiffArgs()): Either[String, QuickDiffArgs] = {
    argList match {
      ...
      case srcArg :: dstArg :: tail => parseArgs(tail, quickDiffArgs.copy(srcPath = Some(srcArg), dstPath = Some(dstArg)))
