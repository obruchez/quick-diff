  ...
  def diffs(srcDir: File, dstDir: File): Either[String, Diffs] = {
    ...
    def combineSubDiffs(
      ...
      errorOrCurrentDiffs match {
        ...
        case Right(currentDiffs) =>
          subDirectoryPairs match {
    ...
    combineSubDiffs(commonDirectories.toList).right map { subDirectoriesDiffs =>
      ...
      val outOfDateFiles =
        if (checkDates) {
          sameSizeFilePairs.foldLeft(JavaFileListPair()) { (outOfDateFiles, filePair) =>
            {
              ...
              JavaFileListPair(
                if (srcLastModified < dstLastModified) {
                  filePair.src :: outOfDateFiles.src
                } else {
                  outOfDateFiles.src
                },
                if (dstLastModified < srcLastModified) {
                  filePair.dst :: outOfDateFiles.dst
                } else {
                  outOfDateFiles.dst
                }
              )
      ...
      .fold(
        ...
        quickDiffArgs => {
          ...
          diffsEither.fold(
            ...
            diffs => {
              if (diffs.noDifference) {
                println("No difference was detected")
              } else {
                diffs.toStrings.foreach(println)
              }
  ...
  private def directoryPathOptionAsFile(
    ...
    pathOption map { path =>
      ...
      if (directoryAsFile.isDirectory) {
        Right(directoryAsFile)
      } else {
        Left(path + " is not a valid directory")
      }
    } getOrElse {
      Left(emptyPathError)
    }
