...
p. Several methods in the project return an _Either[String, T]_ instead of a _T_. By convention, in Scala, the left part
of _Either_ is often used to return an error (_Throwable_, _String_, etc.). Then, different _Either[Error, T]_ values
can be chained using _Either.RightProjection.flatMap_, _Either.RightProjection.map_, or, more simply, using for
comprehensions.
...
p. This way, error management can be deferred until the latest moment. To explicitly process errors, _Either.fold_ can
be used.
...
p. Or, using regular pattern matching:
